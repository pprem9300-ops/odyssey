...
**Current phase:** `P17 LIVE — D26 "Oxygen" design + motion refactor (palette + one-ticker Lenis + cinematic cold-boot + masked reveals + breathing aura) PLUS D27 (bespoke per-view pins for Lung Lab / Week / Fuel, desktop-only). Lenis smooth-scroll on desktop (wheel/trackpad); mobile uses NATIVE touch momentum (D27.1 — syncTouch:false, after Lenis-driven touch stuttered on the phone). engine/data untouched, no console errors. ▶ D28/D29 — bug scan + FIXES SHIPPED: clusters A (scroll-trigger lifecycle regression) · B (engine NaN-on-partial-profile) · C (new-device data-loss) · D (smaller) all fixed & verified; only E (last-writer-wins sync, architectural) remains. See "🐛 KNOWN BUGS". (Push notifications: declined.)`
**Overall progress:** ▰▰▰▰▰▰▰ ~99% (all features + the D26/D27 design overhaul + D29 bug fixes — live). Cache-bust now `app.js?v=28` / `css?v=24`, `sw odyssey-v28` (motion `?v=14`, engine `?v=9`, gate `?v=7`, chart `?v=5`, cloud `?v=5`, exercises `?v=4`).
...
### ✅ BUG SCAN — A/B/C/D FIXED & SHIPPED (D29, 2026-06-26, `edcf702`); only E remains

The D28 scan's clusters **A–D are fixed, verified & shipped** (`?v=28` / `sw odyssey-v28`; engine node-tested across empty/partial/null/cutter, ACWR cold-start gated, and the scroll-trigger leak confirmed gone — **3 triggers after 6 view switches**). Detail in `DECISIONS.md` **D29**. ⚠️ **Live-confirm** the desktop behaviours the frozen preview can't run: no scroll-jump when logging on Week/Lab/Fuel, smooth pins, and the mobile parallax. The bullets below now read as *what was fixed*; **only 📐 E is still open**:
