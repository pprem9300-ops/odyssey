...
**Current phase:** `P11 — AWWWARDS REFACTOR · auth LIVE · hero+pastel palette+streak-reset+exercise-tracker DONE & pushed. Next: scroll Journey + perf profiling`
**Overall progress:** ▰▰▰▰▰▰▰ ~95% (auth live; hero, premium pastel palette, streak reset, detailed exercise tracker all shipped; remaining: scroll-driven Journey, perf profiling at 4× CPU, polish)
...
1. **SCROLL-LAG profiling + view-transition feel** — particle field is gone (main cost) and the clip-path wipe was already replaced with a reliable `autoAlpha` fade (`transitionView` in motion.js). Remaining quick wins: `content-visibility:auto`+`contain-intrinsic-size` on offscreen sections, convert any `.meter`/`.bar` width-anims → `transform:scaleX`, batch reveals; then **profile at 4× CPU throttle** to confirm 60fps. Confirm the fade feels good (tune `DUR`).
2. **Scroll-driven JOURNEY** — designed (DECISIONS D19); `initJourneyScroll()` is a STUB in app.js (called from `switchView('journey')`). Pinned progress rail filling to real streak fraction + scrubbed milestone reveals; drop the pin on mobile (`gsap.matchMedia('(min-width:769px)')`).
3. **Plan-generator surface** — engine `generateWeek` + Week-view split picker already exist + the tracker logs against them; optionally add a richer "plan" surface (volume/progression suggestions from `workoutLog`).
4. **Other (my discretion):** wire `--lilac` into the sleep/readiness card; per-day checklist persistence by date (extend `workoutLog` pattern to the breath/cardio/hydrate rows); water + breathwork-minutes logging; weekly insight; nutrition-regimes "Regimes" view (proven lean-gain/cut).
