...
**Current phase:** `P11 — AWWWARDS REFACTOR (in progress) · auth LIVE · hero+pastel palette+streak-reset DONE. Next: exercise tracker + scroll Journey + perf profiling`
**Overall progress:** ▰▰▰▰▰▰▱ ~92% (auth live; hero redesigned, premium pastel palette applied, streak reset added; remaining: exercise tracker/plan-gen, scroll Journey, perf profiling, verify view-transition)
...
1. **DETAILED EXERCISE TRACKER + PLAN GENERATOR** — per-exercise **set/rep/weight logging + history + progression** (engine has `generateWeek`; `exercises.js` = 60-move DB; today's checklist is just checkmarks in the `doneToday` Set, NOT persisted per-date). Add a real tracker (log actual sets/reps/load per exercise per day, show history/PRs) + a stronger plan-generator surface.
2. **SCROLL-LAG profiling** — particle field is gone (main cost). Now run the `awwwards-web-motion` perf checklist: `content-visibility:auto` on offscreen sections, batch reveals, convert `.meter` width-anim → `transform:scaleX`, **profile at 4× CPU throttle** to confirm 60fps. Also **verify/replace `transitionView`** (the clip-path wipe — GSAP `inset()` tween is unreliable; observed collapsing → use an `autoAlpha` crossfade fallback).
3. **Scroll-driven JOURNEY** — designed (DECISIONS D19); `initJourneyScroll()` is a STUB in app.js. Pinned progress rail filling to real streak fraction + scrubbed milestone reveals; drop the pin on mobile.
4. **Other (my discretion):** wire `--lilac` into the sleep card; per-day checklist persistence by date; water + breathwork-minutes logging; weekly insight; nutrition-regimes "Regimes" view (proven lean-gain/cut).
