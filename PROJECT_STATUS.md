...
**Current phase:** `P17 LIVE — D26 "Oxygen" design + motion refactor (palette + one-ticker Lenis + cinematic cold-boot + masked reveals + breathing aura) PLUS D27 (bespoke per-view pins for Lung Lab / Week / Fuel, desktop-only; Lenis now on ALL phones — mobile smooth-scroll, user opt-in). engine/data untouched, no console errors. ⚠️ Lenis-on-mobile + the pins need live confirmation (frozen preview can't run them). See "▶ RESUME HERE". (Push notifications: declined.)`
**Overall progress:** ▰▰▰▰▰▰▰ ~99% (all features + the D26/D27 awwwards-grade design/motion overhaul — live). Cache-bust now `app.js?v=25` / `css?v=24`, `sw odyssey-v25` (motion `?v=12`, chart `?v=5`, engine `?v=8`, exercises `?v=4`).
...
- ⚠️ **Confirm Lenis-on-mobile LIVE (D27):** Lenis now runs on all phones (`syncTouch:true`) — the user opted in over the prior jank history. Test the scroll feel on a real phone (ideally 4× CPU throttle); if it stutters, the one-line dial-back is **`syncTouch:false`** in `motion.js` → `initSmoothScroll` (keeps Lenis on desktop, native momentum on touch). Also confirm the desktop pins (Lab/Week/Fuel) feel right.
