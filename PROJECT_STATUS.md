...
**Current phase:** `P17 LIVE — D26 "Oxygen" design + motion refactor (palette + one-ticker Lenis + cinematic cold-boot + masked reveals + breathing aura) PLUS D27 (bespoke per-view pins for Lung Lab / Week / Fuel, desktop-only). Lenis smooth-scroll on desktop (wheel/trackpad); mobile uses NATIVE touch momentum (D27.1 — syncTouch:false, after Lenis-driven touch stuttered on the phone). engine/data untouched, no console errors. ▶ D28/D29/D30 — bug scan + ALL fixes shipped: clusters A (scroll-trigger lifecycle) · B (engine NaN-on-partial-profile) · C (new-device data-loss) · D (smaller) · **E (cloud-sync merge — union dated logs + recency, converges two devices instead of last-writer-wins)** all FIXED & verified. See "🐛 KNOWN BUGS". (Push notifications: declined.)`
**Overall progress:** ▰▰▰▰▰▰▰ ~99% (all features + the D26/D27 design overhaul + D29/D30 bug fixes — live). Cache-bust now `app.js?v=29` / `css?v=24`, `sw odyssey-v29` (engine `?v=10`, motion `?v=14`, gate `?v=7`, chart `?v=5`, cloud `?v=5`, exercises `?v=4`).
...
### ✅ BUG SCAN — A–E ALL FIXED & SHIPPED (D29 + D30, 2026-06-26)

**All five scan clusters (A–E) are fixed, verified & shipped** (now `?v=29` / `sw odyssey-v29`; engine node-tested, the scroll-trigger leak confirmed gone — 3 triggers after 6 view switches, and the cloud-merge verified by 27 node asserts + a 4-lens adversarial workflow). Detail in `DECISIONS.md` **D29** + **D30**. ⚠️ **Live-confirm** the desktop behaviours the frozen preview can't run: no scroll-jump when logging on Week/Lab/Fuel, smooth pins, and the mobile parallax. The bullets below are *what was fixed*:
...
- **✅ E · Cloud-sync merge (D30):** replaced last-writer-wins with a pure `mergeState` — dated logs UNION (per-date entries from both devices survive; `workoutLog` deep-unions exercises, `equipment`/`mealSwaps` union) + RECENCY for settings (`profile.updatedAt`, stamped in `save()`); two devices converge. 27 node asserts + a 4-lens adversarial workflow (2 HIGH + 1 LOW fixed, 4 refuted). **Known limitation:** array-union can't sync a *deletion* — un-logging a meal / un-ticking on one device resurrects it on the other (kept union since preserving a logged item beats honoring a rare cross-device un-tick; per-item tombstones would be the true fix).
...
- ✅ **Per-view scroll choreography — DONE (D27):** bespoke desktop-only pins + parallax via `initViewScroll` — Lung Lab (lungs scrub *alive*), Week (analytics pin), Fuel (macro-band pin). Other views could get bespoke moments too if wanted.
- ✅ **Mobile scroll resolved (D27.1):** Lenis-driven touch (`syncTouch:true`) stuttered on the phone → flipped to **`syncTouch:false`** — desktop keeps smooth Lenis (wheel/trackpad), mobile uses native iOS momentum, and the per-view parallax/reveals still run on mobile via ScrollTrigger on native scroll. (Lesson re-confirmed: **don't let Lenis drive touch on this app.** Retrying full smooth-touch is the same one line.) Still worth a glance: the desktop pins (Lab/Week/Fuel) feeling right.
...
### ✅ Done 2026-06-22 → 06-26 (P12–P16 — newest first)
- **Persistence · cloud · editable target · measurement-aware calc** (D24): reopen-last-view (localStorage `odyssey.lastView`); **progress photos now sync** (`profile.photoLog`, supersedes the D22 local-only choice); **editable goal weight** (`targetOf(p)` + cockpit stepper); **Katch-McArdle BMR** from lean mass + **body-fat-aware `selectGoal`** + `measurementTrends()` coach directives. 60 node asserts; verified live.
